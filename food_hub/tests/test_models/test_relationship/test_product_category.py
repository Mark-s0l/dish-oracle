import pytest
from django.core.exceptions import ValidationError
from django.core.files.uploadedfile import SimpleUploadedFile

from food_hub.models import Category, Company, Country, Product


@pytest.fixture
def country(db):
    return Country.objects.create(name="Россия")


@pytest.fixture
def company(db, country):
    return Company.objects.create(name="Компания", country=country)


@pytest.fixture
def category(db):
    return Category.objects.create(name="Десерты")


@pytest.fixture
def another_category(db):
    return Category.objects.create(name="Выпечка")


@pytest.fixture
def image_file():
    return SimpleUploadedFile("test.jpg", b"filecontent", content_type="image/jpeg")


class TestProductCompany:
    @pytest.mark.django_db
    def test_product_category_fk_success(
        self, company, category, image_file, save_and_clean
    ):
        product = Product(
            company=company,
            category=category,
            name="Мороженое",
            ean_code="4006381333931",
            img_field=image_file,
        )
        save_and_clean(product)
        from_db = Product.objects.get(pk=product.pk)
        assert from_db.category == category
        assert from_db.category.name == "Десерты"

    @pytest.mark.django_db
    def test_product_category_fk_required(self, company, image_file, save_and_clean):
        product = Product(
            company=company,
            category=None,
            name="Мороженое",
            ean_code="9780306406157",
            img_field=image_file,
        )
        with pytest.raises(ValidationError):
            save_and_clean(product)

    @pytest.mark.django_db
    def test_product_change_category(
        self, company, category, another_category, image_file, save_and_clean
    ):
        product = Product(
            company=company,
            category=category,
            name="Пирожное",
            ean_code="9781861972712",
            img_field=image_file,
        )
        save_and_clean(product)
        product.category = another_category
        save_and_clean(product)
        from_db = Product.objects.get(pk=product.pk)
        assert from_db.category == another_category

    @pytest.mark.django_db
    def test_multiple_products_same_category(
        self, company, category, image_file, save_and_clean
    ):
        product1 = Product(
            company=company,
            category=category,
            name="Мороженое",
            ean_code="5012345678900",
            img_field=image_file,
        )
        product2 = Product(
            company=company,
            category=category,
            name="Эклер",
            ean_code="5901234123457",
            img_field=image_file,
        )
        save_and_clean(product1)
        save_and_clean(product2)
        assert Product.objects.filter(category=category).count() == 2

    @pytest.mark.django_db
    def test_category_delete_cascades_to_products(
        self, company, category, image_file, save_and_clean
    ):
        product = Product(
            company=company,
            category=category,
            name="Мороженое",
            ean_code="9780306406157",
            img_field=image_file,
        )
        save_and_clean(product)
        category.delete()
        assert not Product.objects.filter(pk=product.pk).exists()
